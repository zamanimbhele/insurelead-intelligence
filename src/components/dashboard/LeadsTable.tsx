...
export function LeadsTable({ leads, demoMode = false }: { leads: Lead[]; demoMode?: boolean }) {
  ...
  const filtered = useMemo(() => {
    return leads.filter((l) => {
      ...
      const matchesProduct = productFilter === "all" || l.insuranceProducts.includes(productFilter as Lead["insuranceProducts"][number]);
      return matchesQuery && matchesStatus && matchesProduct;
    });
  }, [leads, productFilter, query, statusFilter]);
      ...
      <div className="flex flex-col gap-3 border-b border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-xs">
          ...
          <input
            ...
            placeholder="Search lead, industry, province..."
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.slice(0, 25).map((lead) => (
              <tr key={lead.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3 text-slate-600">{getInsuranceProductLabels(lead).join(", ")}</td>
